  ...
  return (
    <div className="min-h-screen">
      ...
      <main className={`max-w-2xl mx-auto px-6 ${embedded.isEmbedded ? "py-4" : "py-8"}`}>
        {/* Debug: show raw URL params when embedded (remove after testing) */}
        {embedded.isEmbedded && Object.keys(embedded.rawParams).length > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 text-xs font-mono">
            <p className="font-semibold text-blue-700 mb-1">Embedded URL params:</p>
            {Object.entries(embedded.rawParams).map(([key, value]) => (
              <p key={key} className="text-blue-600">{key} = {value}</p>
            ))}
          </div>
        )}
