# Export-SubmittalsByContractor-GA-Open.ps1
# Filters "Enhanced Register" sheet by each Contractor AND Info="GA" AND QA Code not A or B (open items only)
...
try {
    ...
    # Find header row and key columns
    $headerRow = 7
    $qaCodeCol = 11       # Column K
    $infoCol = 12         # Column L
    ...
    # Get unique contractor values (only rows where Info = "GA" and QA Code not A or B, split MULTIPLE entries)
    $contractors = @{}
    for ($r = $headerRow + 1; $r -le $lastRow; $r++) {
        $info = $ws.Cells($r, $infoCol).Value2
        if ($info -and $info.ToString().Trim() -eq "GA") {
            $qa = $ws.Cells($r, $qaCodeCol).Value2
            $qaVal = if ($qa) { $qa.ToString().Trim() } else { "" }
            if ($qaVal -ne "A" -and $qaVal -ne "B") {
    ...
    Write-Host "Found $($sorted.Count) contractors with open GA submittals" -ForegroundColor Cyan
    ...
    foreach ($contractor in $sorted) {
        ...
        $pdfName = "1 - Submittal Report - GA Open - $safeName.pdf"
        ...
        # Apply filters: Field is relative to the filter range starting column
        # Range starts at column B (2), so QA Code (col 11) is field 10, Info (col 12) is field 11, Contractor (col 14) is field 13
        $filterRange = $ws.Range($ws.Cells($headerRow, 2), $ws.Cells($lastRow, $contractorCol))
        $filterRange.AutoFilter(10, "<>A", 1, "<>B")  # xlAnd = 1, exclude A and B
        $filterRange.AutoFilter(11, "GA")
