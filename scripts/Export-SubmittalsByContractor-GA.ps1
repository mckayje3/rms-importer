# Export-SubmittalsByContractor-GA.ps1
# Filters "Enhanced Register" sheet by each Contractor AND Info="GA", exports filtered view as PDF
...
try {
    ...
    # Get unique contractor values (only rows where Info = "GA", split MULTIPLE entries)
    $contractors = @{}
    ...
    Write-Host "Found $($sorted.Count) contractors with GA submittals" -ForegroundColor Cyan
    ...
    foreach ($contractor in $sorted) {
        ...
        $pdfName = "1 - Submittal Report - GA - $safeName.pdf"
        ...
        # Range starts at column B (2), so Info (col 12) is field 11, Contractor (col 14) is field 13
